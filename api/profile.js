...
// POST /api/profile  body: { farmer_id, name, state, ... }
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    try {
      const r = await fetch(`${url}/rest/v1/farmer_profiles?farmer_id=eq.${farmer_id}&limit=1`, { headers });
  ...
  if (req.method === 'POST') {
    ...
    try {
      const r = await fetch(`${url}/rest/v1/farmer_profiles`, {
        method: 'POST',
